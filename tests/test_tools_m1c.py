"""
Testes das ferramentas do agente CRM.

Como o pytest funciona:
- Descobre automaticamente funções cujo nome começa com `test_`.
...
Sobre `.invoke({...})` nas ferramentas:
- As funções decoradas com `@tool` viram `StructuredTool` (LangChain). A forma recomendada é
